...
def check_for_time_frame(task):
    ...
    if (
        "no" in yes_or_no.choices[0].message.content
        or "No" in yes_or_no.choices[0].message.content
    ):
        return False
...
def call_groq(task):
    ...
    print(chat_completion.choices[0].message.content.split("</think>")[1])
